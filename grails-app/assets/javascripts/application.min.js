...
$.ajax({type:"POST",url:"sendPasswordResetEmail",data:{e:a}}).done(function(b){processResults(b);});}function attemptPasswordReset(){var b=$("#newPassword").val();
var a=$("#verifyNewPassword").val();$("#loginButton .as-button-label").hide();$("#loginButton .spinner").show();$.ajax({type:"POST",url:"attemptPasswordReset",data:{p1:b,p2:a}}).done(function(c){processResults(c);
});}function bookAppointment(){var b=$("#loggedIn").val();var c=$("#email").val();var e=$("#password-Book").val();var d=$("#firstName").val();var f=$("#lastName").val();
var i=$("#phoneNumber").val();var g=$("#emailReminder").is(":checked");var a=$("#textMessageReminder").is(":checked");var h=$("#rememberMe").is(":checked");
$("#loginButton .as-button-label").hide();$("#loginButton .spinner").show();$.ajax({type:"POST",url:"bookAppointment",data:{loggedIn:b,e:c,p:e,f:d,l:f,ph:i,eRmndr:g,tRmndr:a,remember:h}}).done(function(j){processResults(j);
...
$.ajax({type:"POST",url:"cancelAppointment",data:{e:b,p:a}}).done(function(e){processCancelAppointmentResults(e);});}}function disableButton(a){$(a).attr("disabled","disabled");
$(a).find(".as-button-label").hide();$(a).find(".spinner").show();}function getPageContent(b,e,a){var c=$(b).closest(".page").attr("page");if(typeof c!="undefined"&&typeof a!="undefined"){var d="get"+a.charAt(0).toUpperCase()+a.slice(1);
$.ajax({type:"POST",url:d,data:e}).done(function(f){loadPageContent(c,a,f);}).fail(function(){$(b).html("Error");$(b).addClass("error-button");});}}function loadPageContent(c,a,b){$(".main-content").css("background-color","white");
