$(document).ready(function(){getSection("fourteenDayView");});function getTimeSlotOptions(){var a=$("#services").val();var b=$("#dateOfAppointment").val();
if(a!="Choose a service..."){$.ajax({type:"POST",url:"/admin/getTimeSlotOptions",data:{sId:a,aDate:b}}).done(function(c){if(c.indexOf("ERROR")>-1){$("#bookForClientButton").html("Error");
...
}else{$("#timeSlotsForRescheduledAppointment-"+b).html(d);}});}}$(document).on("tap",".main",function(){var a=($(".navbar-collapse").attr("class").indexOf("in")>=0);
if(a){$(".navbar-toggle").click();}});$(document).on("tap",".nav a",function(b){var a=$(this).attr("href").substring(1);if(a==="logout"){window.location.href="/access/logout";
}else{if(a==="clientSite"){window.location.href="/book";}else{getSection(a);$(".navbar-toggle").click();}}});function getSection(a){$("#mask").fadeIn();
$.ajax({type:"POST",url:"/admin/getSection",data:{section:a}}).done(function(b){$(".active").removeClass("active");$('a[href="#'+a+'"]').parent().addClass("active");
$(".main").html(b);$("#mask").fadeOut();});}$(document).on("tap","#addClient",function(a){$.ajax({type:"POST",url:"/admin/getClientDataForm"}).done(function(b){if(b.indexOf("ERROR")>-1){alert("Dang... you broke it.");
...
}});});$(document).on("tap","#editClient",function(b){var a=$("#clientsDetailsSelector").val();$.ajax({type:"POST",url:"/admin/getClientDataForm",data:{cId:a}}).done(function(c){if(c.indexOf("ERROR")>-1){alert("Dang... you broke it.");
...
if(c===-1){$("#clientsDetailsSelector").html(b);}});}$(document).on("tap","#saveClientButton",function(h){var f=$("#e").val();var i=$("#e2").val();var d=$("#p").val();
...
if(j===-1){hideClientRegistrationForm();alert("User saved!");}});}});$(document).on("tap","#cancelClientRegistrationButton",function(a){hideClientRegistrationForm();
...
}});});$(document).on("tap","#saveTextButton",function(b){var a=$("#homepageText").val();$.ajax({type:"POST",url:"/admin/saveHomepageMessage",data:{m:a}});
});$(document).on("tap","#saveClientNotesButton",function(b){var a=encodeURIComponent($("#clientNotes").val());$.ajax({type:"POST",url:"/admin/saveClientNotes",data:{n:a}});
});$(document).on("tap","#blockOffTimeButton",function(c){var a=$("#chooseDateToBlockOff").val();var i=$("#fromHour").val();var f=$("#fromMinute").val();
...
$("#blockOffTimeButton").html($("#waitingSpinner").html());$.ajax({type:"POST",url:"/admin/blockOffTime",data:{date:a,from:h,to:g}}).done(function(e){var l=JSON.parse(e);
...
$("#blockOffTimeButton").addClass("error-button");}});});$(document).on("tap","#blockOffDaysButton",function(a){var c=$("#fromWholeDay").val();var b=$("#toWholeDay").val();
...
});$(document).on("tap",".blocked-time",function(c){var b=$(this).find("input:checkbox");var a=$(this);if(c.target.type!=="checkbox"){$(b).trigger("click");
}var d=$(b).is(":checked");if(d===true){$(a).addClass("deleted");}else{$(a).removeClass("deleted");}});$(document).on("tap","#deleteBlockedTimeslotsButton",function(b){var a=$("#deleteBlockedTimeslotsButton");
...
});$("#dateOfRescheduledAppointment-"+a).on("change",function(){getTimeSlotOptionsForRescheduledAppointment(a);});$("#rescheduleButton-"+a).on("tap",function(){var d=$("#servicesForRescheduledAppointment-"+a).val();
var e=$("#dateOfRescheduledAppointment-"+a).val();var c=$("#timeSlotsForRescheduledAppointment-"+a).val();$("#rescheduleButton-"+a).html($(".spinner").html());
$.ajax({type:"POST",url:"/admin/rescheduleAppointment",data:{aId:a,sId:d,aDate:e,sTime:c}}).done(function(f){var g=JSON.parse(f);if(g.success===true){$("#rescheduleButton-"+a).html("Success");
