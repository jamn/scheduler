function getTimeSlotOptions(){var a=$("#services").val();var b=$("#dateOfAppointment").val();if(a!="Choose a service..."){$.ajax({type:"POST",url:"/admin/getTimeSlotOptions",data:{sId:a,aDate:b}}).done(function(c){if(c.indexOf("ERROR")>-1){$("#bookForClientButton").html("Error");
...
if(a){$(".navbar-toggle").click();}});$(document).on("click",".nav a",function(a){$(".navbar-toggle").click();});function getSection(b){$("#mask").fadeIn();
var a=$(".calendarStartDate").val();console.log(a);$.ajax({type:"POST",url:"/admin/getSection",data:{section:b,calendarStartDate:"calendarStartDate"}}).done(function(c){$(".active").removeClass("active");
$('a[href="#'+b+'"]').parent().addClass("active");$(".main").html(c);$("#mask").fadeOut();});}$(document).on("click","#addClient",function(a){$.ajax({type:"POST",url:"/admin/getClientDataForm"}).done(function(b){if(b.indexOf("ERROR")>-1){alert("Dang... you broke it.");
...
}});});$(document).on("click","#saveTextButton",function(b){var a=$("#homepageText").val();$.ajax({type:"POST",url:"/admin/saveHomepageMessage",data:{m:a}});
});$(document).on("click","#saveClientNotesButton",function(b){var a=encodeURIComponent($("#clientNotes").val());$.ajax({type:"POST",url:"/admin/saveClientNotes",data:{n:a}});
...
});$("#dateOfRescheduledAppointment-"+a).on("change",function(){getTimeSlotOptionsForRescheduledAppointment(a);});$("#rescheduleButton-"+a).on("click",function(){rescheduleAppointment(a);
});}});}function rescheduleAppointment(a){var c=$("#servicesForRescheduledAppointment-"+a).val();var d=$("#dateOfRescheduledAppointment-"+a).val();var b=$("#timeSlotsForRescheduledAppointment-"+a).val();
$("#rescheduleButton-"+a).html($(".spinner").html());$.ajax({type:"POST",url:"/admin/rescheduleAppointment",data:{aId:a,sId:c,aDate:d,sTime:b}}).done(function(e){var f=JSON.parse(e);
if(f.success===true){$("#rescheduleButton-"+a).html("Success");$("#rescheduleButton-"+a).removeClass("error-button");setTimeout(function(){$("#rescheduleAppointmentModal").modal("toggle");
},400);setTimeout(function(){window.location.href="./admin";},1250);}else{$("#rescheduleButton-"+a).html("Error");$("#rescheduleButton-"+a).addClass("error-button");
}});}