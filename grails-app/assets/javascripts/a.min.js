$(document).ready(function(){setTimeout(hideAlert,2000);$("#clearBlockedTimeModal").on("show.bs.modal",function(c){var a=$(c.relatedTarget);var b=$(this);
...
}else{$("#timeSlotsForRescheduledAppointment-"+b).html(d);}});}}$(document).on("click",".book-new-appointment",function(a){var b=$(a.currentTarget).attr("datetime");
alert(b);});$(document).on("click","#addClient",function(a){$.ajax({type:"POST",url:"/admin/getClientDataForm"}).done(function(b){if(b.indexOf("ERROR")>-1){alert("An error has occured. Please try again.");
